...
package io
{
	...
	public class Socket extends EventEmitter
	{
		...
		public function connect():void {
			trace('connecting : ' + connecting + ' | connected : ' + connected);
		...
		public function reconnect():void {
			if (reconnecting && connected) {
				...
				for (var i:String in namespaces) 
					if (i != '') (namespaces[i] as SocketNamespace).packet( new ConnectPacket() );
		...
		public function onDisconnect(reason:String):void {
			...
			trace('Socket :: onDisconnect '  + wasConnected);
			_connected = false;
			_connecting = false;
			_open = false;
		...
		public function onError(err:Object):void {
			if (err is ErrorPacket && (err as ErrorPacket).advice && options.reconnect && 
				(err as ErrorPacket).advice == RECONNECT && connected)
			{
				disconnect();
				reconnect();
			}
			
			publish(ERROR, (err is ErrorPacket ? (err as ErrorPacket).reason : err));
		}
